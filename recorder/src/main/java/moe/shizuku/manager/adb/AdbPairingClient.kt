 ...
 *
 * Local adaptation: direct Conscrypt compile-time reference replaced by
 * moe.shizuku.manager.adb.ConscryptCompat (runtime reflection on the platform
 * provider), so no hidden-API stub dependency is needed.
 */
...
/**
 * JNI bridge to the vendored libadb.so SPAKE2+ implementation extracted at
 * build time from the pinned thedjchi/Shizuku release APK.
 */
